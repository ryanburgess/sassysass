...
module.exports = function page() {
  ...
  prompt.get({
    properties: {
      name: {
        description: strings.namePage,
        conform: function(fileName) {
          return !fs.existsSync(dir + '/' + filePath + '/' + fileName + '.scss');
        },
        message: strings.pageExists
      },
      filePath: {
        description: strings.pathToPage
      },
      modules: {
        description: strings.addModules
      }
    }
  }, function (err, result) {
    ...
    fs.exists(dir + '/' + filePath + '/' + name + '.scss', function (exists) {
      ...
      if(exists !== true) {
        ...
        console.log(strings.pageVersion + version);
      }else {
